...
function parse(q, extension) {
  ...
  for(let i = 0; i < sentences.length; i += 1) {
    ...
    if(sentence === '_and_' || sentence === '_or_') {
      if(i>0) {
        let j = result.sentences.length - 1
        result.sentences[j].operatorRight = sentence.replace(/_/g,'')
      }
      continue
    }
  ...
  for(let i = 1; i < result.sentences.length; i += 1) {
    result.sentences[i].operatorLeft = result.sentences[i-1].operatorRight
  }
